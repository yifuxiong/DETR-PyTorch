...
def gen_sineembed_for_position(pos_tensor):
    '''
    generate sine position embeding for object query
    :param pos_tensor: object query pos
    :return:
    '''
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        # 自定义的MultiheadAttention，最后输出的通道数改了
    ...
    def forward_post(self, tgt, memory,
        ...
        # # (256, 1, 8, 32) cat (256, 1, 8, 32) = (256, 1, 8, 64)
        ...
        k = k.view(hw, bs, self.nhead, n_model // self.nhead)
        # k_pos在channel维度固定住了，这里在通道上进行复制
        k_pos = torch.repeat_interleave(k_pos, repeats=bs, dim=1)
        k_pos = k_pos.view(hw, bs, self.nhead, n_model // self.nhead)
        # (256, 1, 8, 32) cat (256, 1, 8, 32) = (256, 1, 8, 64)
        k = torch.cat([k, k_pos], dim=3).view(hw, bs, n_model * 2)

        tgt2 = self.cross_attn(query=q,  # (100, 1, 512)
                               key=k,  # (256, 1, 512)
                               value=v,  # (256, 1, 256)
