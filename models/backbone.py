...
def build_backbone(pretrained=False, freeze_bn=False, model='r50'):
    ...
    elif model == 'swin_B_256_22k':
        img_size = int(model.split('_')[-2])
        return build_swin_transformer(modelname=model, img_size=img_size), 1024
    elif model == 'mb2':
        return mobilenet_v2(pretrained=pretrained), 2048
    else:
        raise ValueError('There`s no model match your input, check your model: `{}`'.format(model))
...
if __name__ == '__main__':
    ...
    # model = resnet50()
    model, _ = build_backbone(model='mb2')
