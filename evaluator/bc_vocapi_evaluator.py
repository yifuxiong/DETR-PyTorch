...
import torch
from data.bc_voc import VOCDetection, BC_CLASSES
import sys
import os
import time
import numpy as np
...
class VOCAPIEvaluator():
    ...
    def __init__(self, data_dir, device, transform, labelmap=BC_CLASSES, set_type='val', year='2021', display=True):
        ...
        # path
        self.devkit_path = data_dir + 'BC' + year
        self.annopath = os.path.join(data_dir, 'BC2021', 'Annotations', '%s.xml')
        self.imgpath = os.path.join(data_dir, 'BC2021', 'JPEGImages', '%s.png')
        self.imgsetpath = os.path.join(data_dir, 'BC2021', 'ImageSets', 'Main', set_type + '.txt')
        self.output_dir = self.get_output_dir('bc_eval/', self.set_type)

        # dataset
        self.dataset = VOCDetection(data_dir=data_dir,
                                    image_sets=[('2021', set_type)],
                                    transform=None
                                    )
    ...
    def voc_eval(self, detpath, classname, cachedir, ovthresh=0.1, use_07_metric=True):
