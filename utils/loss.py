...
from .distributed_utils import is_dist_avail_and_initialized, get_world_size


def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2):
    '''
    Focal Loss. Loss used in RetinaNet for dense detection:
        https://arxiv.org/abs/1708.02002.
    :param inputs: A float tensor of arbitrary shape.
        The predictions for each example.
    :param targets: A float tensor with the same shape as inputs.
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        :param outputs: a dict that contains at least these entries:
            "pred_logits": Tensor of dim [batch_size, num_queries, num_classes] with the classification logits
            "pred_boxes": Tensor of dim [batch_size, num_queries, 4] with the predicted box coordinates
        :param targets: a list of targets (len(targets) = batch_size), where each target is a dict containing:
        ...
        :param indices: [(i, j), (i, j), ...], len(indices) == batch_size
        return from HungarianMatcher. A list of size batch_size, containing tuples of (index_i, index_j) where:
    ...
    def _get_src_permutation_idx(self, indices):
        ...
        :return:
        '''
        # [len[0, 0, ...]=index_i1, len[1, 1, ...]=index_i2, ...]
